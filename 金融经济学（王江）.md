...
### 1. 为什么要使用“均值-方差”偏好？ (12.1)

在一般情况下，期望效用 $E[u(\tilde{w})]$ 依赖于财富分布的所有矩（均值、方差、偏度、峰度等）。但在本章，我们假设投资者**只关心**财富分布的两个特征：
1.  **期望（Mean, $\bar{w}$）**：代表预期财富水平（越多越好）。
2.  **方差（Variance, $\sigma_w^2$）或标准差（$\sigma_w$）**：代表风险（越低越好）。

这种偏好函数记为 $v(\bar{w}, \sigma_w)$。

**问题：** 这种简化合理吗？我们有两个强有力的理由支持这种简化：

#### 理由一：二次效用函数 (Quadratic Utility)
**定理 12.1**：如果效用函数 $u(\cdot)$ 是二次的，那么期望效用完全由均值和方差决定。

**推导：**
设 $u(w) = w - \frac{1}{2}\alpha w^2$ ($\alpha > 0$)。
$$
\begin{aligned}
E[u(\tilde{w})] &= E\left[ \tilde{w} - \frac{1}{2}\alpha \tilde{w}^2 \right] \\
&= E[\tilde{w}] - \frac{1}{2}\alpha E[\tilde{w}^2]
\end{aligned}
$$
利用统计学公式 $Var(\tilde{w}) = E[\tilde{w}^2] - (E[\tilde{w}])^2$，即 $E[\tilde{w}^2] = \bar{w}^2 + \sigma_w^2$。代入上式：
$$
\begin{aligned}
E[u(\tilde{w})] &= \bar{w} - \frac{1}{2}\alpha (\bar{w}^2 + \sigma_w^2) \\
&= \bar{w} - \frac{1}{2}\alpha \bar{w}^2 - \frac{1}{2}\alpha \sigma_w^2
\end{aligned}
$$
显然，这个式子只包含 $\bar{w}$ 和 $\sigma_w$。

#### 理由二：正态分布 (Normal Distribution)
**定理 12.2**：如果资产回报服从联合正态分布，那么任何风险厌恶型投资者的偏好都可以表示为均值-方差偏好。

**逻辑：** 正态分布完全由均值和方差两个参数确定。既然分布被这两个参数锁定了，那么基于该分布计算出的期望效用自然也只依赖于这两个参数。

---

### 2. 均值—方差前沿组合 (12.2)

现在我们要解决核心问题：**给定N个风险资产，如何构建最优组合？**

#### 2.1 优化问题设定
*   **目标**：在达到目标期望收益率 $\bar{r}_p$ 的前提下，最小化组合方差。
*   **变量**：权重向量 $z = [z_1, ..., z_N]^T$。
*   **约束**：
    1.  收益率约束：$z^T \bar{r} = \bar{r}_p$
    2.  预算约束：$z^T \iota = 1$ （其中 $\iota$ 是全1向量）

**数学表达 (公式 12.2)**：
$$
\begin{aligned}
\min_z \quad & \frac{1}{2} z^T \Sigma z \\
\text{s.t.} \quad & z^T \bar{r} = \bar{r}_p \\
& z^T \iota = 1
\end{aligned}
$$
...
#### 2.2 求解过程（拉格朗日乘子法）
构造拉格朗日函数：
$$ L = \frac{1}{2} z^T \Sigma z + \lambda_1 (\bar{r}_p - z^T \bar{r}) + \lambda_2 (1 - z^T \iota) $$

对 $z$ 求一阶导数并令为0：
$$ \Sigma z - \lambda_1 \bar{r} - \lambda_2 \iota = 0 $$
解得最优权重 $z$：
$$ z = \lambda_1 \Sigma^{-1} \bar{r} + \lambda_2 \Sigma^{-1} \iota $$

这里 $\lambda_1$ 和 $\lambda_2$ 是待定系数，由两个约束条件代入求解可得。
这一结果引出了著名的**两基金分离定理 (Two-Fund Separation Theorem)**。

### 2.3 结论：两基金分离 (定理 12.4)
观察 $z$ 的解结构，它是向量 $\Sigma^{-1}\bar{r}$ 和 $\Sigma^{-1}\iota$ 的线性组合。
这意味着：**任何一个均值-方差有效组合，都可以看作是两个特定的“基准互助基金”的线性组合。**

> **解读**：这对基金行业意义重大。它意味着投资者不需要研究几千只股票，只需要买入两只设计良好的ETF（覆盖整个前沿边界），然后根据自己的风险偏好调整这两只ETF的比例即可。

---

### 3. 前沿组合的性质与零协方差组合 (12.3)

#### 3.1 最小方差边界的形状
如果我们在 $\bar{r} - \sigma$ 平面上画图：
*   所有可能的组合构成一个区域。
*   **前沿（Frontier）** 是双曲线（Hyperbola）。
*   **有效前沿（Efficient Frontier）** 是双曲线的上半部分（即给定风险，收益最高的部分）。
*   左下角的点是**全局最小方差组合 (MVP)**。

#### 3.2 零协方差组合 (Zero-Covariance Portfolio)
这是一个在定价中非常重要的数学性质。
**定理 12.9**：对于前沿上的任意组合 $p$（除了MVP），都存在唯一的另一个前沿组合 $z_{cp}$，使得它们两者互不相关（协方差为0）。

**定价关系 (公式 12.6)**：
如果 $p$ 是有效组合，$z_{cp}$ 是其零协方差组合，那么对于任意资产 $q$，其收益率满足：
$$ \bar{r}_q - \bar{r}_{zcp} = \beta_{p,q} (\bar{r}_p - \bar{r}_{zcp}) $$
其中 $\beta_{p,q} = \frac{Cov(r_q, r_p)}{\sigma_p^2}$。

> **解读**：这其实是CAPM的一般形式（Black's CAPM）。如果 $p$ 是市场组合，且存在无风险资产（此时 $\bar{r}_{zcp} = r_f$），这就是标准的CAPM。

---

### 4. 存在无风险资产的情形 (12.4)

现在我们加入一个**无风险资产**，收益率为 $r_f$。

#### 4.1 几何变化：资本市场线 (CML)
之前的双曲线变成了**直线**。
*   投资者可以将资金在无风险资产和某个风险组合之间分配。
*   所有可行组合中最优的，是从无风险利率点 $(0, r_f)$ 出发，向风险资产的可行域做**切线**。
*   切点就是**切点组合 (Tangency Portfolio, $z_T$)**。

这条直线被称为 **资本市场线 (Capital Market Line, CML)**。

#### 4.2 数学推导
目标是最大化夏普比率（Sharpe Ratio），即直线的斜率：
$$ \max_z \frac{z^T \bar{r} - r_f}{\sqrt{z^T \Sigma z}} $$
(注：这里 $z$ 是风险资产的权重向量，只不过我们这里归一化处理)。

求解一阶条件得到切点组合的权重 $z_T$ (公式 12.7 的推论)：
$$ z_T \propto \Sigma^{-1} (\bar{r} - r_f \iota) $$
归一化后：
$$ z_T = \frac{\Sigma^{-1}(\bar{r} - r_f \iota)}{\iota^T \Sigma^{-1}(\bar{r} - r_f \iota)} $$

#### 4.3 再次分离 (Two-Fund Separation)
**定理 12.11**：当存在无风险证券时，任何有效组合都可以由**无风险证券**和**切点组合 $z_T$** 两个资产构成。

> **解读**：这就是所谓的“托宾分离定理”。你的风险偏好只决定你持有多少现金（无风险资产），而不改变你持有的风险资产内部的比例（永远持有切点组合 $z_T$）。在均衡状态下，这个 $z_T$ 就是市场组合。

---

### 5. 风险与收益的线性关系 (Beta的诞生)

这是本章的终点，也是连接下一章CAPM的桥梁。

**定理 12.13**：如果 $p$ 是一个有效组合（比如切点组合），那么对于任意资产 $q$，我们有：
$$ \bar{r}_q - r_f = \beta_{q,p} (\bar{r}_p - r_f) $$
其中：
$$ \beta_{q,p} = \frac{Cov(\tilde{r}_q, \tilde{r}_p)}{Var(\tilde{r}_p)} $$

**推导思路 (12.9 - 12.10)**：
利用一阶最优条件，我们知道切点组合 $z_T$ 满足 $z_T \propto \Sigma^{-1}(\bar{r} - r_f \iota)$。
将其代入协方差定义，经过代数变形即可得到上述线性关系。

这个公式告诉我们：
1.  **特质风险不被定价**：资产 $q$ 的方差 $\sigma_q^2$ 并不直接出现在公式里。
2.  **只有系统性风险被定价**：决定资产 $q$ 预期超额收益的，只有它与有效组合 $p$ 的协方差（即 $\beta$）。
...
CAPM 是一个**市场均衡模型**。它的核心思想在于：如果所有投资者都根据均值-方差准则进行投资（即选择均值-方差有效组合，MVE），那么在市场达到均衡时，资产的预期收益率与其所承担的**系统性风险（市场风险）**之间存在一个简单而优雅的线性关系。

---

### 1. 市场组合 (The Market Portfolio)

在讨论资产的需求后，我们必须引入资产的供给。

#### 1.1 定义
如果我们将市场上交易的所有证券（包括股票和债券）进行加总，我们就得到了资产的总供给。这个资产的总集合本身就是一个组合，我们称之为**市场组合 (Market Portfolio, 记为 $M$)**。

#### 1.2 数学表达
假设市场上有 $N$ 种风险资产。
*   令 $\bar{\theta}_n$ 为资产 $n$ 的发行总量（或总股数）。
*   市场组合的物理数量向量为：
    $$ \theta_M = \begin{bmatrix} \bar{\theta}_1 \\ \vdots \\ \bar{\theta}_N \end{bmatrix} $$
*   令 $P = [P_1, \cdots, P_N]^T$ 为所有交易证券的价格向量。
*   资产 $n$ 的市值为 $v_n = P_n \bar{\theta}_n$。
*   **市场总市值 (Market Capitalization)** 为：$v_M = P^T \theta_M = \sum_{n=1}^N v_n$。
*   **市值权重**：证券 $n$ 在市场组合中的相对市值权重为 $z_{M,n} = v_n / v_M$。

因此，用组合权重表示的市场组合向量为：
$$ z_M = \begin{bmatrix} P_1 \bar{\theta}_1 / v_M \\ \vdots \\ P_N \bar{\theta}_N / v_M \end{bmatrix} $$

---

### 2. 市场均衡 (Market Equilibrium)

给定参与者的需求和市场的供给，我们来考察市场均衡。为了简单起见，我们首先假设**存在无风险证券**。

在存在无风险证券时，两基金分离定理告诉我们：每一个参与者对风险资产的需求都是**切点组合 (Tangency Portfolio, 记为 $T$)** 的形式，区别仅仅在于投资金额的多少。

#### 2.1 定理 13.1（核心基础）
> **在均值—方差偏好下，市场达到均衡时，市场组合就是切点组合。即 $z_M = z_T$。**

#### 2.2 详细推导与证明
1.  **设定变量**：假设有 $K$ 个参与者。令 $a_k$ 为参与者 $k$ 对切点组合 $T$ 的投资需求量（金额）。令 $w_k$ 为参与者 $k$ 用于投资的总财富。
2.  **无风险资产投资**：参与者 $k$ 对无风险资产的投资量自然为 $w_k - a_k$。
3.  **风险资产市场出清条件**：所有参与者对风险组合（切点组合）的需求总和，必须等于市场上风险资产的总市值（即市场组合）。
    $$ \sum_{k=1}^K a_k z_T = \left( \sum_{k=1}^K a_k \right) z_T = v_M z_M $$
4.  **无风险资产市场出清条件**：假设无风险资产是零供给的（即内部借贷，借出总额等于借入总额），则要求：
    $$ \sum_{k=1}^K (w_k - a_k) = 0 $$
    这推导出一个重要结论：
    $$ \sum_{k=1}^K a_k = \sum_{k=1}^K w_k $$
5.  **宏观经济恒等式**：经济体中所有投资者的总财富，必须等于所有资产的总市值（因为财富就是由这些资产构成的）。因此：
    $$ \sum_{k=1}^K w_k = v_M $$
6.  **得出结论**：将步骤4和5代入步骤3中，我们得到：
    $$ v_M z_T = v_M z_M $$
    消去 $v_M$，证明完毕：
    $$ z_T = z_M $$

**经济学直觉**：既然所有人都只买切点组合和无风险资产，那么市场上所有的风险资产加起来（市场组合），其内部的相对比例必须和切点组合一模一样，否则市场无法出清。

---

### 3. CAPM 给出的资产定价关系

既然市场组合 $M$ 就是切点组合 $T$，而切点组合是均值-方差有效的。我们直接利用有效组合的数学性质，可以得到 CAPM 的核心定价公式。

#### 3.1 定理 13.2 (标准 CAPM 公式)
> **对于任意证券 $n$，其期望收益率满足：**
> $$ \bar{r}_n - r_F = \beta_{n,M} (\bar{r}_M - r_F) $$
> **其中，$\beta_{n,M} = \frac{Cov[\tilde{r}_n, \tilde{r}_M]}{Var[\tilde{r}_M]}$ 是证券 $n$ 的市场 $\beta$ 值。**

#### 3.2 证券市场线 (SML) 与风险分解
上述等式被称为**证券市场线 (Security Market Line, SML)**。它表明，资产的风险溢价与它对市场组合的 $\beta$ 值成正比。比例系数 $(\bar{r}_M - r_F)$ 是市场组合的风险溢价，也称为**单位风险的风险溢价**。

为了更深入理解，我们将资产收益率 $\tilde{r}_n$ 对市场收益率 $\tilde{r}_M$ 作线性回归（市场模型）：
$$ \tilde{r}_n - r_F = \alpha_n + \beta_{n,M}(\tilde{r}_M - r_F) + \tilde{\epsilon}_n $$
这里 $E[\tilde{\epsilon}_n] = 0$ 且 $Cov[\tilde{r}_M, \tilde{\epsilon}_n] = 0$。

**CAPM 的严苛要求**：CAPM 要求回归的截距项 $\alpha_n$ **必须为 0**（即不存在超额收益）。

**方差（总风险）的分解**：
对上述回归方程两边取方差，我们得到：
$$ \sigma_n^2 = \beta_{n,M}^2 \sigma_M^2 + \sigma_{\epsilon,n}^2 $$
*   **系统风险 (Systematic Risk, $\beta_{n,M}^2 \sigma_M^2$)**：由市场波动引起的风险。**只有这部分风险有溢价补偿。**
*   **非系统风险/剩余风险 (Idiosyncratic/Residual Risk, $\sigma_{\epsilon,n}^2$)**：资产特有的风险。**这部分风险可以通过分散投资消除，因此市场不给予溢价补偿。**

---

### 4. 拓展：不存在无风险资产的 CAPM (Zero-Beta CAPM)

如果市场中不存在无风险资产（即 Black CAPM），均值-方差前沿 (MVF) 是一条双曲线。
*   市场组合 $M$ 仍然是一个 MVF 组合（因为它是所有投资者持有的 MVF 组合的加权平均）。
*   定价公式变为：
    $$ \bar{r}_q - \bar{r}_{zcm} = \beta_{q,M} (\bar{r}_M - \bar{r}_{zcm}) $$
    其中，$\bar{r}_{zcm}$ 是与市场组合协方差为 0 的 MVF 组合（零贝塔组合）的期望收益率。标准 CAPM 只是零贝塔组合恰好为无风险资产时的一个特例。

---

### 5. 高级推导：作为一般均衡结果的 CAPM

我们在前面是从“均值-方差偏好”直接推导。现在我们从更基础的微观经济学假设（效用函数或分布假设）出发，严格推导 CAPM。

#### 推导 A：基于二次型效用函数的推导
假设参与者 $k$ 具有二次型效用函数：$u_k(c_k) = u_{k,0}(c_{k,0}) + a_k/2 (c_{k,1} - \bar{c}_k)^2$。

1.  **一阶条件 (Euler Equation)**：参与者组合选择的最优化一阶条件为：
    $$ E[u'_{k,1}(\tilde{c}_{k,1})(\tilde{r}_q - r_F)] = 0 $$
2.  **代入边际效用**：因为效用是二次型的，其导数是线性的 $u'_{k,1} \propto (\tilde{c}_{k,1} - \bar{c}_k)$。代入得到：
    $$ E[(\tilde{c}_{k,1} - \bar{c}_k)(\tilde{r}_q - r_F)] = 0 $$
    $$ E[\tilde{c}_{k,1} - \bar{c}_k] E[\tilde{r}_q - r_F] + Cov[\tilde{c}_{k,1}, \tilde{r}_q] = 0 $$
3.  **加总至市场水平**：对所有参与者求和。令总消费 $\tilde{C}_1 = \sum \tilde{c}_{k,1}$。
    $$ -(E[\tilde{C}_1] - \bar{C})(\bar{r}_q - r_F) = Cov[\tilde{C}_1, \tilde{r}_q] $$
4.  **市场出清**：总消费必须等于市场组合的总支付，即 $\tilde{C}_1 = v_M(1 + \tilde{r}_M)$。代入上式：
    $$ \bar{r}_q - r_F = \frac{v_M}{-(E[\tilde{C}_1] - \bar{C})} Cov[\tilde{r}_M, \tilde{r}_q] $$
5.  **求解系数**：令资产 $q$ 为市场组合 $M$，上式变为 $\bar{r}_M - r_F = \frac{v_M}{-(...)} \sigma_M^2$。求出系数并代回，即可得到标准 CAPM 公式。

#### 推导 B：基于正态分布与 CARA 效用函数的推导
假设资产支付 $\tilde{v} \sim N(\bar{v}, \Sigma)$，投资者具有常数绝对风险厌恶 (CARA) 效用函数 $u(w) = -e^{-aw}$。

1.  **最优化问题转化**：在正态分布和 CARA 效用下，最大化期望效用严格等价于均值-方差最优化：
    $$ \max_{\theta_k} E[\tilde{w}_k] - \frac{a_k}{2} Var[\tilde{w}_k] $$
    其中期末财富期望 $E[\tilde{w}_k] = w_k(1+r_F) + \theta_k^T[\bar{v} - (1+r_F)P]$，方差 $Var[\tilde{w}_k] = \theta_k^T \Sigma \theta_k$。
2.  **求解最优需求**：对 $\theta_k$ 求导得一阶条件：
    $$ \bar{v} - (1+r_F)P = a_k \Sigma \theta_k \implies \theta_k = \frac{1}{a_k} \Sigma^{-1} [\bar{v} - (1+r_F)P] $$
3.  **应用市场出清**：总需求等于总供给 $\sum \theta_k = \bar{\theta}$。
    $$ \left( \sum_{k=1}^K \frac{1}{a_k} \right) \Sigma^{-1} [\bar{v} - (1+r_F)P] = \bar{\theta} $$
4.  **推导均衡价格**：定义综合风险容忍度 $1/\alpha \equiv \sum 1/a_k$。解出价格向量 $P$：
    $$ P = \frac{1}{1+r_F} (\bar{v} - \alpha \Sigma \bar{\theta}) $$
    *(公式 \**)*： $\bar{v} - (1+r_F)P = \alpha \Sigma \bar{\theta}$
5.  **转换为收益率 (CAPM 形式)**：
    资产 $n$ 的收益率 $\tilde{r}_n = \tilde{v}_n/P_n - 1$。期望超额收益率写为：
    $$ \bar{r}_n - r_F = (\alpha / P_n) (\Sigma \bar{\theta})_n $$
    利用市场组合市值 $w_M = \bar{\theta}^T P$，并计算协方差 $Cov[\tilde{r}_M, \tilde{r}_n]$ 和 $Var[\tilde{r}_M]$，经过代数化简（两边同乘 $\bar{\theta}^T$ 求出 $\alpha$ 的表达式），最终可以严格推导出：
    $$ \bar{r}_n = r_F + \beta_{n,M} (\bar{r}_M - r_F) $$
...
**套利定价理论 (Arbitrage Pricing Theory, 简称 APT)**，由 Stephen Ross (1976) 提出。它提供了一个更为一般化的视角：我们不再依赖特定的效用函数，而是**直接从资产收益的统计结构出发，运用“无套利”这一金融学最基础的原则来推导定价关系**。

---

### 1. 资产收益风险的因子模型 (The Factor Model)

APT 的出发点是一个纯统计的假设：资产收益率由几个共同的风险因子驱动。

我们假设 $N$ 个资产的收益风险可以用以下 **线性因子模型 (Linear Factor Model)** 描述：
$$ \tilde{r}_n = \bar{r}_n + \sum_{k=1}^F b_{n,k} \tilde{f}_k + \tilde{\epsilon}_n, \quad n = 1, 2, \cdots, N $$

**模型假设：**
1.  **零均值：** $E[\tilde{f}_k] = E[\tilde{\epsilon}_n] = 0, \forall n, k$ (因子和特质冲击的期望为0)。
2.  **单位方差与有界残差：** $E[\tilde{f}_k^2] = 1$, 且 $E[\tilde{\epsilon}_n^2] = \sigma_n^2 < \bar{\sigma}^2$ (残差方差有上限)。
3.  **不相关性：**
    *   因子之间不相关：$E[\tilde{f}_k \tilde{f}_{k'}] = 0, \forall k \neq k'$
    *   因子与残差不相关：$E[\tilde{f}_k \tilde{\epsilon}_n] = 0$
    *   残差之间互不相关（弱化假设下可以是对角阵或稀疏矩阵）：$E[\tilde{\epsilon}_n \tilde{\epsilon}_{n'}] = 0, \forall n \neq n'$

**术语解释：**
*   $\bar{r}_n$: 资产 $n$ 的期望收益率。
*   $\tilde{f}_k$: 共同风险因子（系统性风险的来源）。
*   $b_{n,k}$: 资产 $n$ 在因子 $k$ 上的**因子载荷 (Factor Loading)**，类似于CAPM中的 $\beta$。
*   $\tilde{\epsilon}_n$: 资产特有的**剩余风险/特质风险 (Idiosyncratic Risk)**。

---

### 2. 精确因子模型与套利定价

为了建立直觉，我们首先考虑一个简化的特例：**不存在特质风险 ($\tilde{\epsilon}_n = 0, \forall n$)**。我们称之为“精确因子模型”。

#### 2.1 精确单因子模型推导

假设只有一个风险因子：$\tilde{r}_n = \bar{r}_n + b_n \tilde{f}$。

**【引理 14.1 & 14.2】存在一个无风险组合和一个因子载荷为1的组合。**
*详细推导步骤：*
考虑任意两个资产 $i$ 和 $j$，其因子载荷 $b_i \neq b_j$ 且都不为0。构造一个投资组合，权重分别为 $\omega$ 和 $1-\omega$。
组合收益率为：
$$ \tilde{r}_p = \omega \tilde{r}_i + (1-\omega) \tilde{r}_j =[\omega \bar{r}_i + (1-\omega) \bar{r}_j] +[\omega b_i + (1-\omega)b_j]\tilde{f} $$

**第一步：构造无风险组合 $p_0$**
令组合的因子载荷为0，即 $\omega b_i + (1-\omega)b_j = 0$。
解得权重： $\omega = \frac{b_j}{b_j - b_i}$
代回期望收益部分，由于无风险，其收益必须等于无风险利率 $r_F$（假设存在）：
$$ r_F = \frac{b_j}{b_j - b_i}\bar{r}_i + \left(1 - \frac{b_j}{b_j - b_i}\right)\bar{r}_j = \frac{b_j \bar{r}_i - b_i \bar{r}_j}{b_j - b_i} $$
将等式移项变形，我们可以得到一个深刻的结论：
$$ \frac{\bar{r}_i - r_F}{b_i} = \frac{\bar{r}_j - r_F}{b_j} \equiv \lambda $$
这意味着，**所有资产的“超额收益/因子载荷”比值必须是一个常数 $\lambda$。** $\lambda$ 与具体的资产 $i, j$ 无关。

**第二步：得出定价公式 (Theorem 14.1)**
由上式直接变形可得精确单因子模型下的 APT 定价公式：
$$ \bar{r}_n = r_F + b_n \lambda $$
这里 $\lambda$ 称为**因子溢价 (Factor Premium)**。

**【无套利证明法】**
如果上述公式不成立，比如存在资产 $n$ 使得 $\bar{r}_n > r_F + b_n \lambda$。我们如何套利？
1.  **买入** 1元的资产 $n$。
2.  **卖空** $b_n$ 元的纯因子组合（载荷为1，预期收益为 $r_F+\lambda$）。
3.  **卖空（借入）** $1-b_n$ 元的无风险资产。
*   **净投资额**：$1 - b_n - (1-b_n) = 0$ (零成本)。
*   **未来绝对支付**：
    $$ \tilde{r}_n - b_n(r_F + \lambda + \tilde{f}) - (1-b_n)r_F $$
    展开 $\tilde{r}_n = \bar{r}_n + b_n \tilde{f}$：
    $$ = \bar{r}_n + b_n \tilde{f} - b_n r_F - b_n \lambda - b_n \tilde{f} - r_F + b_n r_F $$
    $$ = \bar{r}_n - r_F - b_n \lambda $$
    因为我们假设 $\bar{r}_n > r_F + b_n \lambda$，所以最终支付 **严格大于0**！
这就是一个完美的无风险零成本套利。由于市场不允许套利，公式必须严格成立。

#### 2.2 精确多因子(F因子)模型

若扩展到多个因子：$\tilde{r} = \bar{r} + b\tilde{f}$ （写成向量形式）。
根据无套利原理，如果一个组合 $z$ 满足：
1. 成本为0：$z^T \iota = 0$ ($\iota$ 为全1向量)
2. 因子风险为0：$z^T b = 0$ (对每一个因子列向量正交)
那么由于没有残差风险，该组合收益确定，因此期望收益必须为0，即 $z^T \bar{r} = 0$。

在数学上，如果 $z$ 与 $\iota$ 和所有 $b_k$ 向量正交，就必然导致 $z$ 与 $\bar{r}$ 正交。根据线性代数的投影定理，$\bar{r}$ 必然可以由 $\iota$ 和 $b_k$ 线性表出：
$$ \bar{r} = \lambda_0 \iota + \sum_{k=1}^F b_k \lambda_k $$
（若存在无风险资产，则 $\lambda_0 = r_F$）。

---

### 3. 核心 APT 理论：包含特质风险与极限套利

现实中，资产总是包含特质风险 $\tilde{\epsilon}_n \neq 0$。此时即使消除了因子风险，组合仍有极小的残差风险，不能应用绝对的“无套利”推导。我们需要引入 **极限套利 (Asymptotic Arbitrage)** 的概念。

**极限套利思想：** 随着资产数量 $N \to \infty$，我们可以通过充分分散化，把特质风险 $\tilde{\epsilon}$ 降到0。

### 3.1 定理 14.3 (Ross APT 界限定理)
如果资产收益由带有有界剩余风险的 F 因子模型描述，且不存在极限套利机会，那么存在常数 $\lambda_0^{(n)}$ 和 $\lambda^{(n)}$ 使得定价误差平方和有界：
$$ \sum_{i=1}^n (\bar{r}_i^{(n)} - \lambda_0^{(n)} - b_i^{(n)T} \lambda^{(n)})^2 \leq A $$
这意味着，**对于绝大多数资产，近似 APT 定价关系是成立的**。

#### 3.2 详细数学证明过程 (高阶推导)
*同学们，这段证明是金融学中最经典的纯数学证明之一，请集中注意力跟上思路。*

**步骤1：定义投影与误差**
我们将期望收益向量 $\bar{r}^{(n)}$ 投影到全1向量 $\iota$ 和因子载荷矩阵 $b^{(n)}$ 构成的空间上。得到残差向量 $\eta^{(n)}$：
$$ \bar{r}^{(n)} = \lambda_0^{(n)} \iota + b^{(n)} \lambda^{(n)} + \eta^{(n)} $$
由正交投影的性质，残差向量 $\eta^{(n)}$ 与投影底空间正交：
$$ \eta^{(n)T} \iota = 0 \quad \text{且} \quad \eta^{(n)T} b^{(n)} = \mathbf{0} $$
我们要证明的是，定价误差的平方和 $Q^{(n)} \equiv \eta^{(n)T} \eta^{(n)}$ 是有界的。

**步骤2：反证法构造套利组合**
假设结论不成立，即当 $n \to \infty$ 时，定价误差爆炸：$Q^{(n)} \to \infty$。
我们利用误差向量本身来构造一个组合，定义权重向量：
$$ x^{(n)} = h^{(n)} \eta^{(n)} $$
其中 $h^{(n)}$ 是一个缩放标量（我们待会儿再决定它等于多少）。
我们检查这个组合的性质：
*   **成本：** $x^{(n)T} \iota = h^{(n)} \eta^{(n)T} \iota = 0$ （这是一个零成本投资）。
*   **因子风险：** $x^{(n)T} b^{(n)} = h^{(n)} \eta^{(n)T} b^{(n)} = 0$ （这是一个消除了所有系统风险的组合）。

**步骤3：计算期望收益和方差**
*   **期望收益 (Mean)：**
    $$ \mu^{(n)} = E[x^{(n)T} \tilde{r}^{(n)}] = x^{(n)T} \bar{r}^{(n)} = h^{(n)} \eta^{(n)T} (\lambda_0^{(n)} \iota + b^{(n)} \lambda^{(n)} + \eta^{(n)}) $$
    由于正交性，前两项消去，剩下：
    $$ \mu^{(n)} = h^{(n)} \eta^{(n)T} \eta^{(n)} = h^{(n)} Q^{(n)} $$
*   **组合方差 (Variance)：**
    因为组合没有因子风险，所以只有特质风险的方差。特质风险的协方差阵 $\Sigma$ 对角线元素有上界 $\bar{\sigma}^2$。
    $$ (\sigma^{(n)})^2 = Var[x^{(n)T} \tilde{\epsilon}^{(n)}] = h^{(n)2} \eta^{(n)T} \Sigma \eta^{(n)} \leq h^{(n)2} \eta^{(n)T} (\bar{\sigma}^2 I) \eta^{(n)} = h^{(n)2} Q^{(n)} \bar{\sigma}^2 $$

**步骤4：施展“魔法”确定缩放系数**
既然我们假设 $Q^{(n)} \to \infty$，我们精心挑选一个收敛速度介于 $Q^{-1}$ 和 $Q^{-1/2}$ 之间的缩放系数，例如：
$$ h^{(n)} = (Q^{(n)})^{-2/3} $$
将这个系数代入上面的期望和方差式子中：
*   期望收益：$\mu^{(n)} = (Q^{(n)})^{-2/3} \cdot Q^{(n)} = (Q^{(n)})^{1/3}$。因为 $Q^{(n)} \to \infty$，所以 **$\mu^{(n)} \to \infty$**。
*   组合方差：$(\sigma^{(n)})^2 \leq ((Q^{(n)})^{-2/3})^2 \cdot Q^{(n)} \cdot \bar{\sigma}^2 = (Q^{(n)})^{-1/3} \cdot \bar{\sigma}^2$。因为 $Q^{(n)} \to \infty$，所以 **$(\sigma^{(n)})^2 \to 0$**。

**步骤5：得出矛盾**
我们刚刚构造了一个零成本投资组合，随着 $n \to \infty$，它的风险（方差）趋于0，但它的预期收益却趋于无穷大！这在经济学上被称为存在 **极限套利机会 (Asymptotic Arbitrage)**。
但这与定理的前提条件“不存在极限套利机会”相矛盾。
**结论：** 因此假设不成立，$Q^{(n)}$ 必须是有界的。即对于大多数资产，APT等式严格成立。

---

### 4. 作为一般均衡结果的 APT (APT in Equilibrium)

纯粹的统计套利（极限套利）有其局限性，它只能证明“近似”成立。但是，如果我们在经济结构上加上一些限制，APT 可以作为一般均衡（如我们在第2章学习的框架）的直接结果而**精确成立**（即所有资产定价误差为0）。

**【定理 14.5】在以下假设下，APT 精确成立：**
1. 资产支付呈现因子结构 $\tilde{x} = \bar{x} + b_x \tilde{f} + \tilde{\epsilon}$。
2. 因子可由证券支付线性表出（市场能张成因子）。
3. 投资者消费可由证券表出。
4. 特别关键：**特质风险 $\tilde{\epsilon}$ 与因子 $\tilde{f}$ 的相关性足够弱，乃至特质风险生成的空间与因子空间完全正交。**

**均衡推导的核心思路（Euler方程的应用）：**
在一般均衡中，任何参与者的一阶最优条件（欧拉方程）必定满足：
$$ 0 = E[u'_{i,1} (\tilde{r}_n - r_F)] $$
利用协方差分解技巧：
$$ 0 = E[u'_{i,1}](\bar{r}_n - r_F) + Cov[u'_{i,1}, \tilde{r}_n] $$
把 $\tilde{r}_n = \bar{r}_n + b_n \tilde{f} + \tilde{\epsilon}_n$ 代入协方差中：
$$ \bar{r}_n - r_F = \frac{-Cov[u'_{i,1}, b_n \tilde{f} + \tilde{\epsilon}_n]}{E[u'_{i,1}]} $$
因为我们的经济结构假设使得参与者的边际效用 $u'_{i,1}$ 仅仅取决于系统性因子 $\tilde{f}$（因为充分分散化后个人消费不包含个别公司的特质冲击），这就意味着 **$u'_{i,1}$ 与资产 $n$ 的特质冲击 $\tilde{\epsilon}_n$ 协方差为0**：$Cov[u'_{i,1}, \tilde{\epsilon}_n] = 0$。

此时公式简化为：
$$ \bar{r}_n - r_F = b_n \left( \frac{-Cov[u'_{i,1}, \tilde{f}]}{E[u'_{i,1}]} \right) $$
令括号内的项为向量 $\lambda$，我们完美地得到了：
$$ \bar{r}_n - r_F = b_n \lambda $$
此时，不仅没有近似误差，而且对所有的资产 $n$ 都绝对成立。

---

### 5. 本章总结

1.  **CAPM vs. APT:** CAPM 规定了单一的市场风险因子，且依赖于均值方差偏好；APT 指出只要收益率存在线性因子结构，通过无套利原则即可得出多因子定价模型，假设更为宽松。
2.  **定价逻辑:** APT 的核心在于，资产的预期超额收益完全由其承担的**系统性因子风险的载荷 ($b_k$)** 及其对应的**因子风险溢价 ($\lambda_k$)** 决定。特质风险由于可以分散掉，不产生风险溢价。
3.  **数学基石:** 对于带有特质风险的普遍情况，Ross 的定理通过构造方差趋于0而收益爆炸的组合反证法，确立了极限套利无利可图时，APT定价误差必须有界的深刻结论。